      ...
      {jsonData?.blocks?.map((block: any, index: number) => {
        switch (block.type) {
          case "paragraph":
            return (
              <div
                className={`pb-3 mt-3   ${style} font-space `}
            ...
            return (
              ...
              >
                {block.data.items.map((item: any, itemIndex: number) => (
                  <li key={itemIndex} className="w-full  line-clamp-2  ">
                    <p className="pb-3  text-sm  text-gray-400 font-space ">
                      {item}
                    </p>
                  </li>
                ))}
